...
K_MSGQ_DEFINE(display_action_msgq, sizeof(disp_action_t), DISP_ACTION_ARRAY_SIZE, 4);
...
void display_action_work_handler(struct k_work *work)
{
	...
	while (k_msgq_num_used_get(&display_action_msgq)) {
		...
		rc = auxdisplay_cursor_position_set(aux_dev, AUXDISPLAY_POSITION_ABSOLUTE, 0,
						    action.line);
		if (rc != 0) {
